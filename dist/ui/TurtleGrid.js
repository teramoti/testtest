...
export class TurtleGrid {
    ...
    setTileSelectHandler(handler) {
        this.tileSelectHandler = handler;
    }
    setRestartHandler(handler) {
        this.restartHandler = handler;
    }
    setBoostHandlers(startHandler, endHandler) {
    ...
    setBoostActive(active) {
        this.drawRoundedPanel(this.boostButton.background, -86, -28, 172, 56, 18, active ? 0xf29d38 : 0x0f4251, 1, active ? 0xffd79c : 0x79d3df, 2);
        this.boostButton.label.setText(active ? 'BOOST ON' : 'BOOST');
        this.boostButton.label.setColor(active ? '#072d38' : '#dff9ff');
    }
    sync(snapshot, timeLeft = this.latestTimeLeft) {
        this.latestSnapshot = snapshot;
        this.latestTimeLeft = timeLeft;
        const tileCatalog = new Map(snapshot.tileCatalog.map((tile)=>[
                tile.id,
                tile
            ]));
    ...
    renderSlide(snapshot, moveResult, onComplete) {
        if (!moveResult.moved || moveResult.to === null || moveResult.from === null) {
            onComplete();
            return;
        }
        const tileView = this.tileViews.get(moveResult.tileId);
        if (tileView === undefined) {
            onComplete();
            return;
        }
        const target = this.getCellCenter(moveResult.to);
        const deltaX = target.x - tileView.container.x;
        const deltaY = target.y - tileView.container.y;
        const slideDirection = this.getTravelDirection(moveResult.from, moveResult.to);
        const tileBaseScale = (GameConfig.TILE_SIZE - 12) / 128 * (this.isTileMovable(tileView.position) ? 1.03 : 1);
        const useCornerSlideFx = tileView.tile.kind === 'corner';
        this.killTweens(tileView.container);
        this.killTweens(tileView.tileSprite);
        const ghost = this.spawnTileGhost(tileView);
        if (useCornerSlideFx) {
            tileView.slideFx.setAngle(slideDirection === Direction.Up || slideDirection === Direction.Down ? 90 : slideDirection === Direction.Left ? 180 : 0);
        ...
        this.scene.time.delayedCall(GameConfig.SLIDE_TWEEN_MS, ()=>{
            this.scene.tweens.add({
                ...
                onComplete: ()=>{
                    ...
                    this.emitBubbleBurst(target.x, target.y, 0x9de7f5, 6);
                    onComplete();
                }
            });
        });
    }
    renderTurtleStep(snapshot, stepResult, onComplete) {
    ...
    celebrateRouteConnect(position) {
    ...
    showRouteClosure(from, blockedPosition, direction) {
    ...
    updateScore(snapshot) {
        ...
        this.multiplierText.setText(`NO MISS x${snapshot.noMissMultiplier.toFixed(2)}   LINK x${snapshot.connectionMultiplier.toFixed(2)}   EFF ${snapshot.efficiency}`);
        const awardSignature = `${snapshot.score}-${snapshot.lastEventLabel}-${snapshot.lastAward}`;
        if (snapshot.lastAward > 0 && awardSignature !== this.awardSignature) {
            this.awardSignature = awardSignature;
            this.awardText.setText(`${snapshot.lastEventLabel}  +${snapshot.lastAward}`);
    ...
    updateBattle(snapshot) {
        ...
        if (snapshot.state === 'ahead') {
            this.battleLeadText.setText(`LEAD +${snapshot.scoreDiff}`);
            this.battleLeadText.setColor('#dfffcf');
        } else if (snapshot.state === 'behind') {
            this.battleLeadText.setText(`BEHIND ${Math.abs(snapshot.scoreDiff)}`);
        ...
        this.drawRoundedPanel(this.battleGaugePlayer, 0, 0, Math.max(18, 268 * snapshot.playerGaugeRatio), 12, 6, 0x8fe5eb, 1);
        this.drawRoundedPanel(this.battleGaugeRival, 268 - Math.max(18, 268 * snapshot.rivalGaugeRatio), 0, Math.max(18, 268 * snapshot.rivalGaugeRatio), 12, 6, snapshot.state === 'behind' ? 0xf08f78 : 0xc17a74, 0.95);
        if (battleChanged) {
            this.pulseBattleHud(snapshot.state);
        }
    }
    updateCombo(snapshot) {
        if (snapshot.comboActive) {
            this.comboText.setText(snapshot.comboCount >= GameConfig.MAX_FLOW_COUNT ? 'FLOW MAX' : `FLOW x${snapshot.comboCount}`);
    ...
    updateTimer(seconds) {
        ...
        const fillColor = seconds <= GameConfig.TIME_DANGER_SECONDS ? 0xf26a5f : seconds <= GameConfig.TIME_WARNING_SECONDS ? 0xf2bb4b : 0x66d2da;
    ...
    showCallout(title, body, accentColor = 0x8fe5eb, durationMs = 760) {
    ...
    showResult(snapshot, battleSnapshot) {
        const battleResult = battleSnapshot.result === 'win' ? 'VICTORY' : battleSnapshot.result === 'lose' ? 'DEFEAT' : 'DRAW';
        this.resultPanel.scoreText.setText(`${battleResult}  ${battleSnapshot.scoreDiff >= 0 ? '+' : ''}${battleSnapshot.scoreDiff}`);
        this.resultPanel.statsText.setText(`YOU ${this.formatScore(snapshot.score)}   RIVAL ${this.formatScore(battleSnapshot.rivalScore)}`);
        this.resultPanel.detailText.setText(`宝石 ${snapshot.jewelCount}   最長 ${snapshot.longestSafeRoute}   救済 ${snapshot.rescueCount}   ループ ${snapshot.loopCount}`);
    ...
    createBackdrop() {
        ...
        for(let index = 0; index < 4; index += 1){
            const ribbon = this.scene.add.ellipse(180 + index * 280, 520 + index % 2 * 64, 240, 38, 0x9adce8, 0.12);
            this.scene.tweens.add({
                targets: ribbon,
                x: ribbon.x + 80,
    ...
    createBoardBase() {
        ...
        for(let row = 0; row < GameConfig.BOARD_SIZE; row += 1){
            for(let col = 0; col < GameConfig.BOARD_SIZE; col += 1){
                ...
                this.drawRoundedPanel(cell, center.x - GameConfig.TILE_SIZE / 2 + 4, center.y - GameConfig.TILE_SIZE / 2 + 4, GameConfig.TILE_SIZE - 8, GameConfig.TILE_SIZE - 8, 24, 0x0e4255, 0.9, 0x2f7084, 2);
            }
        }
    }
    createBlankMarker() {
        ...
        this.drawRoundedPanel(glow, -GameConfig.TILE_SIZE / 2 + 12, -GameConfig.TILE_SIZE / 2 + 12, GameConfig.TILE_SIZE - 24, GameConfig.TILE_SIZE - 24, 22, 0x8fe5eb, 0.08, 0xc8fbff, 2);
    ...
    createTileViews(snapshot) {
        for (const tile of snapshot.tileCatalog){
            ...
            container.setInteractive(new Phaser.Geom.Rectangle(-GameConfig.TILE_SIZE / 2, -GameConfig.TILE_SIZE / 2, GameConfig.TILE_SIZE, GameConfig.TILE_SIZE), Phaser.Geom.Rectangle.Contains);
    ...
    createResultPanel() {
        ...
        restartButtonBg.setInteractive(new Phaser.Geom.Rectangle(-104, 102, 208, 46), Phaser.Geom.Rectangle.Contains);
    ...
    createBoostButton() {
        ...
        container.setInteractive(new Phaser.Geom.Rectangle(-86, -28, 172, 56), Phaser.Geom.Rectangle.Contains);
        container.on('pointerdown', ()=>{
    ...
    refreshTileStates(snapshot) {
        ...
        const nextKey = snapshot.routePreview.nextPosition === null ? '' : `${snapshot.routePreview.nextPosition.row}-${snapshot.routePreview.nextPosition.col}`;
    ...
    drawTile(tileView, isMovable, isConnected, isDanger, isNext, isCurrent, hasJewel, riskLevel) {
        const fillColor = isDanger ? 0x8b3443 : isCurrent ? 0x125b6f : isMovable ? 0x1895aa : 0x0f5567;
        ...
        this.drawRoundedPanel(tileView.plate, -GameConfig.TILE_SIZE / 2 + 6, -GameConfig.TILE_SIZE / 2 + 6, GameConfig.TILE_SIZE - 12, GameConfig.TILE_SIZE - 12, 24, fillColor, isMovable ? 0.72 : 0.52, strokeColor, isCurrent || isMovable ? 4 : 2);
        this.drawRoundedPanel(tileView.nextRing, -GameConfig.TILE_SIZE / 2 + 1, -GameConfig.TILE_SIZE / 2 + 1, GameConfig.TILE_SIZE - 2, GameConfig.TILE_SIZE - 2, 28, 0xfff3bc, 0.08, 0xfff2c0, 3);
        this.drawRoundedPanel(tileView.dangerGlow, -GameConfig.TILE_SIZE / 2 - 4, -GameConfig.TILE_SIZE / 2 - 4, GameConfig.TILE_SIZE + 8, GameConfig.TILE_SIZE + 8, 30, 0xffa391, 0.08, 0xffc0b4, 3);
    ...
    drawTilePath(graphics, tile, color, lineWidth, alpha) {
        graphics.clear();
    ...
    drawSpecialAura(graphics, tile, connected) {
        graphics.clear();
        let visible = false;
        ...
        if (tile.kind === 'branch') {
            visible = true;
            const preferredDirection = tile.branchBias === 'left' ? this.getDirectionVector((tile.rotation + 3) % 4) : this.getDirectionVector((tile.rotation + 1) % 4);
    ...
    refreshRouteLine(snapshot) {
        this.boardRouteGlow.clear();
        this.boardRouteLine.clear();
        const points = [
            this.getCellCenter(snapshot.turtlePosition)
        ];
    ...
    refreshThreatFrame(snapshot) {
        ...
        const strokeColor = snapshot.routePreview.riskLevel === 'danger' || snapshot.routePreview.riskLevel === 'critical' ? 0xffb3a2 : 0xffe2a8;
    ...
    refreshRouteHud(snapshot) {
        ...
        if (preview.loopDetected) {
            ...
            this.routeDetailText.setText(preview.nextJewelDistance === null ? 'ループ成立中' : `宝石まで ${preview.nextJewelDistance} マス`);
            return;
        }
        ...
        if (preview.riskLevel === 'danger') {
            ...
            this.routeDetailText.setText(preview.nextJewelDistance === null ? '猶予が少ない状態です' : `宝石まで ${preview.nextJewelDistance} マス`);
            return;
        }
        if (preview.riskLevel === 'warning') {
            this.routeText.setText(`WATCH ${preview.safeStepCount}`);
            this.routeText.setColor('#fff0cb');
            this.routeDetailText.setText(preview.nextJewelDistance === null ? '分岐の読み直しが必要です' : `宝石まで ${preview.nextJewelDistance} マス`);
            return;
        }
        this.routeText.setText(`SAFE ${preview.safeStepCount}`);
        this.routeText.setColor('#dcffff');
        this.routeDetailText.setText(preview.nextJewelDistance === null ? '次の宝石は未接続です' : `宝石まで ${preview.nextJewelDistance} マス`);
    }
    refreshWarningBanner(snapshot) {
        const preview = snapshot.routePreview;
        let message = '';
    ...
    applyTurtleDirection(direction) {
    ...
    isTileMovable(position) {
        return this.latestSnapshot.movablePositions.some((movablePosition)=>movablePosition.row === position.row && movablePosition.col === position.col);
    }
    spawnTileGhost(tileView) {
        const ghost = this.scene.add.image(tileView.container.x, tileView.container.y, this.getTileTexture(tileView.tile));
        const tileBaseScale = (GameConfig.TILE_SIZE - 12) / 128 * (this.isTileMovable(tileView.position) ? 1.03 : 1);
    ...
    toggleLoopTween(target, active, type, tileView) {
        const field = type === 'danger' ? 'dangerTween' : type === 'current' ? 'currentTween' : 'nextTween';
    ...
    drawRoundedPanel(graphics, x, y, width, height, radius, fillColor, alpha, strokeColor, strokeWidth = 0) {
        graphics.clear();
    ...
    spawnAmbientBubble() {
        const bubble = this.scene.add.circle(Phaser.Math.Between(48, GameConfig.GAME_WIDTH - 48), Phaser.Math.Between(730, 780), Phaser.Math.Between(3, 7), 0xd9f9ff, Phaser.Math.FloatBetween(0.15, 0.34));
        this.scene.tweens.add({
            targets: bubble,
    ...
    emitBubbleBurst(x, y, color, count) {
