...
export class TurtleProto extends Phaser.Scene {
    ...
    handleTileSelected(position) {
        ...
        const rescuedRoute = !this.isDangerRisk(afterSnapshot.routePreview.riskLevel) && this.isDangerRisk(beforeSnapshot.routePreview.riskLevel);
        ...
        this.audioDirector.playSlide(Math.max(0, afterSnapshot.routePreview.safeStepCount - beforeSnapshot.routePreview.safeStepCount), routeImproved);
        if (routeImproved) {
    ...
    handleCrash(from, blockedPosition, direction) {
    ...
    refreshTurtleSpeed(nextSpeedLevel) {
    ...
    restartTurtleTimerIfRunning() {
        if (this.sessionEnded || this.retryPending || this.turtleEvent === null) {
            return;
        }
        this.startTurtleTimer();
    }
    getCurrentTurtleDelay(speedLevel) {
        const baseDelay = Math.max(GameConfig.MIN_TURTLE_STEP_MS, GameConfig.TURTLE_STEP_MS - speedLevel * GameConfig.TURTLE_STEP_ACCELERATION_MS);
        if (!this.boosting) {
            return baseDelay;
        }
        return Math.max(GameConfig.MIN_BOOST_TURTLE_STEP_MS, baseDelay - GameConfig.BOOST_TURTLE_STEP_REDUCTION_MS);
    }
    endSession() {
        if (this.sessionEnded) {
            return;
    ...
    getDangerLevel(riskLevel, timeLeft, battleSnapshot) {
        const routeLevel = riskLevel === 'critical' ? 3 : riskLevel === 'danger' ? 2 : riskLevel === 'warning' ? 1 : 0;
        const timeLevel = timeLeft <= GameConfig.TIME_DANGER_SECONDS ? 3 : timeLeft <= GameConfig.TIME_WARNING_SECONDS ? 1 : 0;
        const battleLevel = battleSnapshot !== null && battleSnapshot.state === 'behind' ? timeLeft <= 12 ? 2 : timeLeft <= 22 ? 1 : 0 : 0;
        return Math.max(routeLevel, timeLevel, battleLevel);
    }
    isRouteImproved(beforeSnapshot, afterSnapshot) {
        return afterSnapshot.routePreview.safeStepCount > beforeSnapshot.routePreview.safeStepCount || !this.isDangerRisk(afterSnapshot.routePreview.riskLevel) && this.isDangerRisk(beforeSnapshot.routePreview.riskLevel) || !beforeSnapshot.routePreview.loopDetected && afterSnapshot.routePreview.loopDetected;
    }
    isDangerRisk(riskLevel) {
        return riskLevel === 'danger' || riskLevel === 'critical';
    }
    getSessionProgress() {
        return 1 - this.timeLeft / GameConfig.SESSION_SECONDS;
    }
    beginRoundIntro(snapshot) {
    ...
    applyBattleSnapshot(snapshot, silent = false) {
    ...
    maybeAnnounceBattleShift(previousSnapshot, nextSnapshot) {
        let signature = '';
        let title = '';
        let body = '';
